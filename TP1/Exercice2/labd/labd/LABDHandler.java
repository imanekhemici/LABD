...
public class LABDHandler extends DefaultHandler {
	...
	public void startElement(String nameSpaceURI, String localName, String rawName, Attributes attributs) throws SAXException {
		...
		for (int index = 0; index < attributs.getLength(); index++) { // on parcourt la liste des attributs
			...
			if(localName.equals("maison")){System.out.println("Maison "+attributs.getValue(index) +":") ;}

			if(attributs.getLocalName(index).equals("surface-m2")){
			superficie += Float.parseFloat(attributs.getValue(index));
		}
			if(localName.equals("alcove")){superficie -= Float.parseFloat(attributs.getValue(index));}
